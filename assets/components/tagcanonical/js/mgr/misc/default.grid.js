tagCanonical.grid.Default = function (config) {
    config = config || {};
    ...
    Ext.applyIf(config, {
        ...
        baseParams: {},
        cls: config['cls'] || 'main-wrapper tagcanonical-grid',
        autoHeight: true,
        paging: true,
        remoteSort: true,
        ...
        viewConfig: {
            ...
            getRowClass: function (rec) {
                ...
                if (rec.data['published'] != undefined && rec.data['published'] == 0) {
                    cls.push('tagcanonical-row-unpublished');
                }
                if (rec.data['active'] != undefined && rec.data['active'] == 0) {
                    cls.push('tagcanonical-row-inactive');
                }
                if (rec.data['deleted'] != undefined && rec.data['deleted'] == 1) {
                    cls.push('tagcanonical-row-deleted');
                }
                if (rec.data['required'] != undefined && rec.data['required'] == 1) {
                    cls.push('tagcanonical-row-required');
                }
                return cls.join(' ');
            }
        },
    });
    tagCanonical.grid.Default.superclass.constructor.call(this, config);
...
Ext.extend(tagCanonical.grid.Default, MODx.grid.Grid, {
    ...
    getSearchField: function (width) {
        return {
            xtype: 'tagcanonical-field-search',
            width: width || 250,
            listeners: {
                search: {
                    fn: function (field) {
    ...
    getMenu: function (grid, rowIndex) {
        ...
        var menu = tagCanonical.utils.getMenu(row.data['actions'], this, ids);
...
Ext.reg('tagcanonical-grid-default', tagCanonical.grid.Default);