...
class tagCanonical
{
    ...
    {
        $this->modx =& $modx;
        $corePath = MODX_CORE_PATH . 'components/tagcanonical/';
        $assetsUrl = MODX_ASSETS_URL . 'components/tagcanonical/';
        ...
        $this->modx->addPackage('tagcanonical', $this->config['modelPath']);
        $this->modx->lexicon->load('tagcanonical:default');
        ...
        switch ($ctx) {
            ...
            default:
                if (!defined('MODX_API_MODE') || !MODX_API_MODE) {
                    ...
                    if ($css = $this->modx->getOption('tagcanonical_frontend_css')) {
                        $this->modx->regClientCSS(str_replace($config['pl'], $config['vl'], $css));
                    }

                    $config_js = preg_replace(array('/^\n/', '/\t{5}/'), '', '
							tagCanonical = {};
							tagCanonicalConfig = ' . $this->modx->toJSON($this->config) . ';
					');


                    $this->modx->regClientStartupScript("<script type=\"text/javascript\">\n" . $config_js . "\n</script>", true);
                    if ($js = trim($this->modx->getOption('tagcanonical_frontend_js'))) {
    ...
    public function runProcessor($action = '', $data = array())
    {
        ...
        $processorsPath = !empty($this->config['processorsPath'])
            ...
            : MODX_CORE_PATH . 'components/tagcanonical/processors/';
